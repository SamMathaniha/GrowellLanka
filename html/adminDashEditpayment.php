...
if (isset($_GET['Pay_ID'])) {
    $paymentIdToEdit = $_GET['Pay_ID'];
    
    // Retrieve payment details from the database
    $selectQuery = "SELECT * FROM payment WHERE Pay_ID = $paymentIdToEdit";
    ...
    if ($result) {
        ...
        if (!$paymentData) {
            // Redirect to the payment details page if payment not found
            header("Location: adminDashViewPayment.php");
            exit;
        }
    } else {
        $error = "Error fetching payment details: " . mysqli_error($conn);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateQuery = "UPDATE payment SET
                    ...
                    date = '$date',
                    amount = '$amount'
                    WHERE Pay_ID = $paymentIdToEdit";
    
    if (mysqli_query($conn, $updateQuery)) {
        // Redirect to the payment details page after successful update
        header("Location: adminDashViewPayment.php");
        exit;
    } else {
        $error = "Error updating payment details: " . mysqli_error($conn);
    }
}
?>
  ...
  <div class="main" >
    ...
    <form method="POST" action="#">
       <h3>Update Payment Details</h3>
        ...
        <label>Cus_ID</label>
        <input type="text" name="cus_ID" value="<?php echo $paymentData['cus_ID']; ?>" required><br><br>

        <label>Email:</label>
        <input type="email" name="email" value="<?php echo $paymentData['email']; ?>" required><br><br>

        <label>Reference No:</label>
        <input type="text" name="refNo" value="<?php echo $paymentData['refNo']; ?>" required><br><br>

        <label> Date:</label>
        <input type="date" name="date" value="<?php echo $paymentData['date']; ?>" required><br><br>

        <label>amount:</label>
        <input type="text" name="amount" value="<?php echo $paymentData['amount']; ?>"><br><br>
