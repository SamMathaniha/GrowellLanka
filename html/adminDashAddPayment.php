...
if(isset($_POST['submit'])){
    ...
    $select = "SELECT * FROM payment WHERE email = '$email' && refNo = '$refNo'";
    $result = mysqli_query($conn, $select);

    if(mysqli_num_rows($result) > 0){
        $error[] = 'This Payment already exists!';
    } else {
        $insert = "INSERT INTO payment(cus_ID, email, refNo, date, amount ) VALUES('$cus_ID', '$email', '$refNo', '$date', '$amount')";
...
<div class="form-input">
  <input type="text" id="departmemailent" name="email"><br>
</div>
</div>

<div class="form-container">
<label class="form-label" for="email">Reference No:</label>
<div class="form-input">
