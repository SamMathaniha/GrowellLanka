...
// Fetching vacancyrequest details from the database
$select = "SELECT * FROM vacancyrequest";
$result = mysqli_query($conn, $select);

// Initialize vacancyrequest array
$vacancyrequest = array();

if ($result) {
    // Store the fetched data in the vacancyrequest array
    while ($row = mysqli_fetch_assoc($result)) {
        $vacancyrequest[] = $row;
    }
}

// Delete vacancyrequest record if the delete button is clicked
if (isset($_POST['delete_vacancyrequest'])) {
    $vacancyrequestIdToDelete = $_POST['delete_vacancyrequest'];
    $deleteQuery = "DELETE FROM vacancyrequest WHERE VacReq_ID = $vacancyrequestIdToDelete";
    mysqli_query($conn, $deleteQuery);
    // After deleting, redirect to the same page to refresh the vacancyrequest details
...
<body >
  ...
  <div class="main" >
    ...
    <table class="customer-table">
        ...
        <tbody>
            <?php foreach ($vacancyrequest as $JobReq): ?>
                <tr>
                    <td><?php echo isset($JobReq['VacReq_ID']) ? $JobReq['VacReq_ID'] : 'N/A'; ?></td>
                    <td><?php echo isset($JobReq['name']) ? $JobReq['name'] : 'N/A'; ?></td>
                    <td><?php echo isset($JobReq['gender']) ? $JobReq['gender'] : 'N/A'; ?></td>
                    <td><?php echo isset($JobReq['age']) ? $JobReq['age'] : 'N/A'; ?></td>
                    <td><?php echo isset($JobReq['department']) ? $JobReq['department'] : 'N/A'; ?></td>
                    <td><?php echo isset($JobReq['PhoneNo']) ? $JobReq['PhoneNo'] : 'N/A'; ?></td>
                    <td>
                        <form action="#" method="POST">
                            <button type="submit" name="delete_vacancyrequest" value="<?php echo $JobReq['VacReq_ID']; ?>" style="background-color: red; color: white;">Delete</button>
                        </form>
                    </td>
                </tr>
            <?php endforeach; ?>
