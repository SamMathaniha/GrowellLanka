...
if (isset($_GET['cus_ID'])) {
    $clientIdToEdit = $_GET['cus_ID'];
    
    // Retrieve client details from the database
    $selectQuery = "SELECT * FROM customer WHERE cus_ID = $clientIdToEdit";
    $result = mysqli_query($conn, $selectQuery);
    
    $clientData = mysqli_fetch_assoc($result);
    
    if (!$clientData) {
        // Redirect to the client details page if client not found
        header("Location: adminDashViewClientDetails.php");
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateQuery = "UPDATE customer SET
                    ...
                    WHERE cus_ID = $clientIdToEdit";
    
    if (mysqli_query($conn, $updateQuery)) {
        // Redirect to the client details page after successful update
        header("Location: adminDashViewClientDetails.php");
        exit;
    } else {
  ...
  <div class="main" >
    ...
    <form method="POST" action="#">
       <h3>Update Client Details</h3>
        ...
        <label>Email:</label>
        <input type="email" name="email" value="<?php echo $clientData['email']; ?>" required><br><br>

        <label>Address:</label>
        <input type="text" name="address" value="<?php echo $clientData['address']; ?>" required><br><br>

        <label>Phone No 01:</label>
        <input type="text" name="PhoneNo1" value="<?php echo $clientData['PhoneNo1']; ?>" required><br><br>

        <label>Phone No 02:</label>
        <input type="text" name="PhoneNo2" value="<?php echo $clientData['PhoneNo2']; ?>"><br><br>

        <label>Description:</label>
        <textarea name="Description"><?php echo $clientData['Description']; ?></textarea><br>
