...
// Fetching review details from the database
...
if ($result) {
    // Store the fetched data in the review array
    while ($row = mysqli_fetch_assoc($result)) {
        $review[] = $row;
    }
}

// Delete review record if the delete button is clicked
if (isset($_POST['delete_review'])) {
    $reviewIdToDelete = $_POST['delete_review'];
    $deleteQuery = "DELETE FROM review WHERE rev_ID = $reviewIdToDelete";
    mysqli_query($conn, $deleteQuery);
    // After deleting, redirect to the same page to refresh the review details
...
<body >
  ...
  <div class="main" >
    ...
    <table class="customer-table">
        ...
        <tbody>
            <?php foreach ($review as $reviewView): ?>
                <tr>
                    <td><?php echo isset($reviewView['rev_ID']) ? $reviewView['rev_ID'] : 'N/A'; ?></td>
                    <td><?php echo isset($reviewView['email']) ? $reviewView['email'] : 'N/A'; ?></td>
                    <td><?php echo isset($reviewView['review_Note']) ? $reviewView['review_Note'] : 'N/A'; ?></td>
                    <td>
                        <form action="#" method="POST">
                            <button type="submit" name="delete_review" value="<?php echo $reviewView['rev_ID']; ?>">Delete</button>
                        </form>
                    </td>
                </tr>
            <?php endforeach; ?>
